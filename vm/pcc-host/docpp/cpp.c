...
#define NCPS	128
#define STATIC          /* host-port: was `static`; extern avoids clashes with K&R non-static forward decls */
...
#undef BUFSIZ   /* host-port: host <stdio.h> sets BUFSIZ=8192, but cpp is sized for 512 */
#ifndef BUFSIZ
#	define BUFSIZ 512
#endif
...
/* host-port: declare x,y as pointers -- K&R implicit-int truncates 64-bit
   pointer args (e.g. np->name) to 32 bits on the host -> fprintf %s SEGV.
   On Kronos char*==int==1 word, so this is a no-op there. */
pperror( s, x, y ) char *s, *x, *y; {
	char *pmsg;
